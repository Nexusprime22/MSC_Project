function makeReflexiveSymmetricTransitive(matrix) {
	const n = matrix.length;
  
	// // Transitive: Update (i, k) to be the sum of (i, j) and (j, k)
	// for (let i = 0; i < n; i++) {
	//   for (let j = 0; j < n; j++) {
	// 	for (let k = 0; k < n; k++) {
	...
	// Reflexive: Add diagonal elements with value 0
	for (let i = 0; i < n; i++) {
		matrix[i][i] = 0;
	}

	// Symmetric: Set (j, i) to the same value as (i, j)
	for (let i = 0; i < n; i++) {
		for (let j = i + 1; j < n; j++) {
  ...
  const resultMatrix = makeReflexiveSymmetricTransitive(inputMatrix);
